...
After an environment has completed CCD decentralisation and its migration rollback window has
closed, add the SDK's example Flyway migration to the application. It removes the application
database's temporary FDW connection to the central CCD database without changing migrated case
data.
...
The migration retains migrated data under the target `ccd` schema and the `postgres_fdw` and
...
The migration additionally refuses to proceed when the staging schema contains an unexpected
...
It is idempotent, including when a previous migration has already removed the foreign tables and
schema but left the server and its credential-bearing user mappings behind.
