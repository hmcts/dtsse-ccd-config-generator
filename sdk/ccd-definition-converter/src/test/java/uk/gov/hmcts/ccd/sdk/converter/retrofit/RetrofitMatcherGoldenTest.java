...
class RetrofitMatcherGoldenTest {
  ...
  void totalsCoverEveryBucket() {
    ...
    // 22 CaseField rows, 1 is a Label -> 21 data-bearing.
    assertThat(report.getTotalDefinitionFields()).isEqualTo(22);
    assertThat(report.getLabelFields()).isEqualTo(1);
    assertThat(report.getDataBearingFields()).isEqualTo(21);

    // 16 exact (incl. the five FixedList-label pin fixtures — labelBearing, annotated, sharedLine,
    // camelConstant, shadowedPin — whose model enums match by name; `state`, the State enum declared
    // as an ordinary field; `oversized`, whose declared enum resolves to the same FixedRadioList
    // the definition declares, the OVERSIZE being a binding question rather than a matching one; and
    // `rivalBand`, whose declared ComplexityBand likewise matches the row's FixedRadioList — the
    // rival CLAIM on that enum being, again, a binding question and not a matching one),
    ...
    assertThat(report.getExactMatches()).isEqualTo(16);
    assertThat(report.getExactMatches() + report.getTypeConflicts()
        + report.getUnmatchedDefinitionFields()).isEqualTo(21);
  }
